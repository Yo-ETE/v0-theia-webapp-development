 ...
 *
 * NOTE: __TX_ID__ est remplace automatiquement par la webapp au flash.
 * (c) 2026 Yoann ETE - THEIA Project
 *
 * Basé sur Doc13 (TX04 - version originale fonctionnelle)
 * Seules modifications :
 * [FIX 1] static const char* TX_ID pour injection webapp
 * [FIX 2] Radio.Send() 1x/s avec timer (conserve delay(200) du Doc13)
 * [FIX 3] checkInactivity() reset apres longue absence
 * Tout le reste IDENTIQUE au Doc13
 */
...
// ================= CONFIG =================
static const char* TX_ID = "__TX_ID__";  // [FIX 1]
...
// ===== OPTION B (mouvement humain) — INCHANGE =====
#define V_MIN_CM_S      8
...
static void checkInactivity(uint32_t now) {
  ...
  bool noActivity = (lastGoodMs == 0) || ((now - lastGoodMs) > INACTIVITY_RESET_MS);
  if (noActivity && havePrev) {
    Serial.println("[INACT] Reset etat (inactivite prolongee)");
    havePrev        = false;
    prevX           = 0;
    prevY           = 0;
    consecutiveMove = 0;
...
void loop() {
  ...
  if (moved) {
    consecutiveMove++;
    lastMoveMs = now;
    lastGoodMs = now;  // pour checkInactivity
    lastX = x_cm;
    lastY = y_cm;
    lastD = d_cm;
    lastV = v_cms;
  ...
  // [FIX 2] Envoi 1x/s — conserve delay(200) du Doc13
  if ((uint32_t)(now - lastSendMs) >= SEND_PERIOD_MS) {
    lastSendMs = now;
    Radio.Send((uint8_t*)txpacket, strlen(txpacket));
    Serial.printf("[TX %s] %s\n", TX_ID, txpacket);
  }

  delay(200);  // INCHANGE — identique Doc13
}
